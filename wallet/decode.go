...
func dump_raw_sigscript(d []byte) bool {
	...
	if p2sh {
		...
		if er==nil {
			var xnyssMode string
			if ms.XnyssMode {
				xnyssMode = "(XNYSS)"
			}

			fmt.Println("      Multisig script", ms.SigsNeeded, "of", len(ms.PublicKeys), xnyssMode)
			...
			if ms.XnyssMode {
				for i := range ms.XnyssSignatures {
					sig := ms.XnyssSignatures[i]

					fmt.Printf("       Signature    = %x....%x\n", sig.SigBytes[:15], sig.SigBytes[len(sig.SigBytes)-15:])
					fmt.Printf("       Public Seed  = %x\n", sig.PubSeed)
					for i := range sig.ChildHashes {
						fmt.Printf("       Child pkh %d  = %x\n", i, sig.ChildHashes[i])
					}
				}
				return len(ms.XnyssSignatures)>=int(ms.SigsNeeded)
			} else {
